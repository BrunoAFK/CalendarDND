...
object DebugLogger {
    ...
    fun buildEngineLog(
        ...
        val nextSummary = if (nextEvent != null) {
            "${nextEvent.title.ifBlank { "(no title)" }} at ${formatDateTime(nextEvent.begin)}"
        } else {
            "none"
        }
        ...
        return buildString {
            ...
            appendLine("------------------------------------------------------------")
            appendLine("Trigger: $trigger (execution ${executionTimeMs}ms)")
            appendLine("Automation: ${if (input.automationEnabled) "enabled" else "disabled"} | DND mode: ${input.dndMode}")
            appendLine("Package: ${context.packageName}")
            appendLine("Calendars monitored: ${formatCalendarIds(input.selectedCalendarIds)}")
            ...
            if (decision.notificationNeeded != null) {
                appendLine("Notification slot: ${decision.notificationNeeded}")
            }
